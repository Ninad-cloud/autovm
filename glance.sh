...
# Backup the original .conf file
	...
	#for checking from keystone...grep -q "^connection = mysql+pymysql" /etc/keystone/keystone.conf || sed -i '0,/^connection = sqlite/ s||connection = mysql+pymysql://keystone:'$COMMON_PASS'@controller/keystone\n#&|' /etc/keystone/keystone.conf
	
	sed -i '/^\[database\]/,/^\[/ {s#^connection = .*#connection = mysql+pymysql://glance:'$COMMON_PASS'@controller/glance#g} ' $filepath1
...
# Backup the original .conf file
	...
	sed -i '/^\[database\]/,/^\[/ {s#^connection = .*#connection = mysql+pymysql://glance:'$COMMON_PASS'@controller/glance#g} ' $filepath2
